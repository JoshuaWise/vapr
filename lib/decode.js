...
/*
	Similar to decodeURIComponent() except a few differences:
	 - Does not decode forward slashes ("/")
	 - If the input is invalid, it returns an empty string instead of an exception
	This module could potentially be replaced with:
		const decode = (str) => { try { return decodeURI(str); } catch (_) { return ''; } };
	Except that the above function does not case-fold non-decoded sequences.
	Also though, I might want to allow package users to utilize reserved
	characters for parsing path parameters (which means not decoding them).
 */
