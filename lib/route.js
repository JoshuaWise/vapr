...
class Route {
	...
	static set catch(catcher) {
		if (typeof catcher !== 'function') throw new TypeError('Expected default route error handler to be a function');
		defaultCatcher = catcher;
	}
}
...
const makeBeforeHandler = (middleware, parent) => (request) => {
	...
	return (function next(returned) {
		...
		if (index < middleware.length) return invokeMiddleware(middleware[index++], request, undefined, next);
		try { return parent(request, undefined); } catch (err) { return err; }
	}());
};

// TODO: maybe experiment with combining beforeHandler and afterHandler
