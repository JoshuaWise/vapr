...
class Route {
	constructor(handler, catcher) {
		...
		const before = [];
		const after = [];
		const beforeHandler = makeBeforeHandler(before, parent.beforeHandler || handler);
		const afterHandlers = [...parent.afterHandlers || [], makeAfterHandler(after)];
		if (!catcher) catcher = parent.catcher || defaultCatcher;
...
// TODO: review this and benchmark it
...
const makeBeforeHandler = (middleware, parent) => (request) => {
	let index = 0;
	return (function next(returned) {
		if (returned != null && returned !== request) return returned;
		if (index < middleware.length) return invoke1(middleware[index++], request, next);
		return invoke1(parent, request);
	}());
};

const makeAfterHandler = (middleware) => (request, response, children, cIndex) => {
	let index = 0;
	return (function next(returned) {
		...
		if (cIndex < children.length) return invoke4(children[cIndex], request, response, children, cIndex + 1);
		return response;
	}());
};
...
// TODO: clean this up and benchmark it
...
const invoke1 = new Function('Promise', `return ${code.replace(/__/g, 'x')}`)(Promise);
const invoke2 = new Function('Promise', `return ${code.replace(/__/g, 'x1, x2')}`)(Promise);
const invoke4 = new Function('Promise', `return ${code.replace(/__/g, 'x1, x2, x3, x4')}`)(Promise);

const defaultCatcher = (_, err) => err;
