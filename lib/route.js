...
const makeHandler = (beforeHandler, afterHandler, catcher) => (req, res) => {
	...
	invokeMiddleware(beforeHandler, request, undefined, function route(response) {
		...
		if (response.error) {
			// TODO: ignore error if the request was aborted
			if (state !== 2) {
				state = 2;
		...
		// TODO: don't send anything if the request was aborted
		// req.resume(); // TODO: do i need this? can it cause unexpected behavior?
