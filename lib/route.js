...
// TODO: create some kind of plugin system on settings,
//       which alters the way execution happens in makeHandler
class Route {
	constructor(...args) {
	...
	// accept/parse([mime, parser]) // TODO: for handling content-type in request.body()
}
...
const makeHandler = (route, handler) => (req, res) => {
	...
	const respond = (value) => {
		if (Response.isResponse(value)) {
			...
			res.writeHead(value.code, value.message, value[shared.hidden].headers);
			res.end();
		} else {
			if (!(value instanceof Error)) value = new TypeError('Route handlers must return Response objects');
			// TODO: fix error loop
			toAsync2(route.onError, request, value).then(respond, respond);
		}
	};
	toAsync1(handler, request).then(respond, respond);
};

const toAsync1 = (fn, arg) => { try { return Promise.resolve(fn(arg)); } catch (err) { return Promise.reject(err); } };
const toAsync2 = (fn, arg1, arg2) => { try { return Promise.resolve(fn(arg1, arg2)); } catch (err) { return Promise.reject(err); } };
