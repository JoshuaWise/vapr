...
class Route {
	constructor(handler, catcher, reporter) {
		...
		if (reporter != null && typeof reporter !== 'function') throw new TypeError('Expected reporter to be a function');
		...
		if (!catcher) catcher = parent.catcher || internalServerError;
		if (!reporter) reporter = parent.reporter || undefined;
		this[shared.settings] = { before, after, beforeHandler, afterHandler, catcher, reporter };
		return Object.setPrototypeOf(makeHandler(beforeHandler, afterHandler(), catcher, reporter), this);
	}
	before(handler) {
...
const groupMiddleware = (middleware) => (parent) => (request, response) => {
	...
	return (function next(returned) {
		...
		if (index === middleware.length) return parent(request, response);
		return invokeMiddleware(middleware[index++], request, response, next);
	}());
};

const invokeMiddleware = (fn, a, b, cb) => {
	let returned, sync;
	try {
		returned = fn(a, b);
