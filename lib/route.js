...
class Route {
	constructor({ logger } = {}) {
		...
		this[shared.settings] = { logger, handlers: [] };
...
const executeRoute = (req, res, logger, handlers, request) => {
	...
	const newResponse = (value) => {
		...
		return (process.nextTick(logger, value), new Response(500)); // Unexpected errors become 500 responses
	};
	...
	const done = () => {
		...
		try {
			...
			try {
				// This should never throw, but we try-catch it just in case.
