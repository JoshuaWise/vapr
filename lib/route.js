...
class Route {
	constructor({ logger } = {}) {
		...
		this[settings] = { logger, handlers: [], setLogger: false };
		return Object.setPrototypeOf(makeRoute(this[settings]), this);
	}
	use(...handlers) {
		for (const handler of handlers) {
			if (Array.isArray(handler)) {
				...
				this[settings].handlers.push(handler);
			}
		}
		return this;
	}
	logger(logger) {
		...
		if (this[settings].setLogger) throw new TypeError('A logger function is already set on this route');
		this[settings].logger = logger;
		this[settings].setLogger = true;
		return this;
	}
	static isRoute(value) {
		return value != null && hasOwnProperty.call(value, settings);
	}
}
