...
const makeHandler = (/*route, */handler) => (req, res) => {
	...
	toAsync(handler, new Request(req, location))
		...
		.then((response) => {
			// TODO
			// if (!Response.isResponse(response)) throw new TypeError('Route handlers must return Response objects');
		});
};

const toResponse = x => Response.isResponse(x) ? x : new Response(500); // TODO: allow custom handling logic for unexpected errors
