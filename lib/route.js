...
class Route {
	constructor(handler, catcher) {
		...
		const afterHandler = (parent.afterHandler || build)(makeAfterHandler(after));
...
const makeHandler = (beforeHandler, afterHandler, catcher) => (req, res) => {
	...
	invoke1(beforeHandler, request, function route(response) {
		...
		if (state === 0) {
			state = 1;
			invoke2(afterHandler, request, response, route);
			return;
		}
...
const makeAfterHandler = (middleware) => (child) => (request, response) => {
	...
	return (function next(returned) {
		...
		if (child) return invoke2(child, request, response); // TODO: benchmark and maybe remove this conditional/invoke2
		return response;
	}());
};
...
const build = parent => child => child ? build(x => parent(child(x))) : parent();
