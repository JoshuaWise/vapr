...
class Route {
	...
	static get report() {
		return defaultReporter === rethrow ? undefined : defaultReporter;
	}
	static set report(reporter) {
		if (reporter != null && typeof reporter !== 'function') throw new TypeError('Expected reporter to be a function');
		defaultReporter = reporter || rethrow;
	}
}
	...
	invokeMiddleware(beforeHandler, request, undefined, function route(response) {
		...
		if (response.error) {
			if (state !== 2) {
				...
				req.destroy();
				/*
					TODO: reporter and defaultReporter should be wrapped when provided
					      to catch errors and rethrow them appropriately
					      also... we could do the same thing for catcher, as long as we
					      maintain the indirection so that the currently active default is
					      looked up properly
				 */
				(reporter || defaultReporter)(response.error);
			}
		} else if (state === 0) {
			state = 1;
...
const rethrow = err => process.nextTick(() => { throw err; });
