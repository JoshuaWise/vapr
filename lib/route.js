...
class Route {
	...
	use(handler) {
		...
		if (typeof catcher !== 'function') throw new TypeError('Expected a handler function');
		if (settings.catcher !== internalServerError) throw new TypeError('A catch function is already set on this route');
		settings.catcher = catcher;
		return this;
	}
	uncatchable(reporter) {
		const settings = this[shared.settings];
		if (typeof reporter !== 'function') throw new TypeError('Expected a handler function');
		if (settings.reporter !== undefined) throw new TypeError('An uncatchable function is already set on this route');
...
const executeRoute = (req, res, settings, handlers, request) => {
	...
	then(invokeBefores(afters, request, handlers), function continueRoute(response) {
		if (request.aborted) {
			if (settings.reporter) process.nextTick(settings.reporter, response.error || new Error('The request was aborted'));
			dropResponse(response);
			req.destroy();
		} else if (response.error) {
			// TODO: figure out what should happen if an unexpected error occurs during the afters
			// TODO: figure out what should happen if an unexpected error occurs inside respond()
			then(invokeCatcher(settings.reporter, request, response.error, settings.catcher), continueRoute);
...
const invokeCatcher = (reporter, request, err, catcher) => {
	return invoke(request => catcher(request, err), request, (returned) => {
		let response = returned != null ? castToResponse(returned)
			: { error: new TypeError(`Route catch handler did not return a response (got ${returned})`) };
		if (response.error) {
			process.nextTick(reporter, response.error);
