...
class Route {
	constructor(fn) {
		...
		this[before] = [];
		this[after] = [];
		this[catcher] = { handler: defaultCatchHandler, changed: false };
		return Object.setPrototypeOf(makeHandler(this[handler], this[before], this[after], this[catcher]), this);
	}
	unexpectedError(handler) {
		if (typeof handler !== 'function') throw new TypeError('Expected unexpectedError() handler to be a function');
		if (this[catcher].changed) throw new TypeError('An unexpectedError() handler was already set on this route');
		this[catcher].handler = handler;
		return this;
	}
	before(handler) {
...
const makeHandler = (handler, before, after, catcher) => (req, res) => {
