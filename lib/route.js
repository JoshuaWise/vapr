...
/*
	...
	reported to the third function passed to the constructor. If such a function
	is not provided, the default bahavior is to throw an uncaught exception,
	crashing the process, unless a static Route.report function is set.
...
class Route {
	...
	static get report() {
		return defaultReporter === thrower ? undefined : defaultReporter;
	}
	static set report(reporter) {
		if (reporter != null && typeof reporter !== 'function') throw new TypeError('Expected reporter to be a function');
		defaultReporter = reporter || thrower;
	}
}
...
const makeHandler = (beforeHandler, afterHandler, catcher, reporter) => (req, res) => {
	...
	invokeMiddleware(beforeHandler, request, undefined, function route(response) {
		...
		if (response.error) {
			// TODO: if this happens because the route handler/catcher tried to
			//       read the body/trailers on an aborted request, perhaps it
			//       should be handled differently
			if (state !== 2) {
				state = 2;
				invokeMiddleware(catcher || defaultCatcher, request, response.error, route);
			} else {
				process.nextTick(reporter || defaultReporter, response.error);
				req.destroy();
			}
		} else if (state === 0) {
			state = 1;
			...
			respond(req, res, response, reporter || defaultReporter);
		}
	});
};
