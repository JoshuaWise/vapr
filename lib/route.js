...
// TODO: support some kind of inheritance and plugin system
class Route {
	constructor(handler) {
...
const makeHandler = (route, handler) => (req, res) => {
	...
	const respond = (value) => {
		if (Response.isResponse(value)) {
			// TODO: writeHead, conditionally write body (validate/sanitize headers)
		} else {
			if (!(value instanceof Error)) value = new TypeError('Route handlers must return Response objects');
			toAsync(route.onError, value).then(respond, respond);
		}
	};
