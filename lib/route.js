'use strict';
const River = require('wise-river');
const Promise = require('wise-promise');
const Request = require('./state/request');
const Response = require('./state/response');
const respond = require('./respond');
const locate = require('./locate');
const shared = require('./shared');

/*
	Routes are server handlers designed to respond to requests, rather than
	route them (contrary to routers).
	
	Besides the primary route handler, a second function can be passed to the
	constructor to handle unexpected errors. When no error handler is given, the
	default bahavior is to simply respond with a 500 status code. It's common to
	provide a custom error handler here for the sake of logging, etc.
	
	Some errors in HTTP are considered unrecoverable. When an error like this
	occurs, the connection will be destroyed immediately, and the error will be
	reported to the third function passed to the constructor, if provided. This
	error handler differs from the previous in that it cannot return a response
	(the error is unrecoverable). This is just used for logging, if desired.
	
	Each route can also have a number of before() and after() middleware hooks.
	If a response is provided in a before() middleware, execution skips directly
	to the after() middlewares. If a response is provided in an after()
	middleware, it replaces the previous response object. Responses must either
	be actual Response objects, or a valid constructor value for creating one.
 */

class Route {
	constructor() {
		const handlers = [];
		this[shared.settings] = { catcher: internalServerError, reporter: undefined, handlers };
		return Object.setPrototypeOf(makeRoute(composeHandlers(handlers), this[shared.settings]), this);
	}
	use(handler) {
		if (arguments.length > 1) throw new TypeError('To pass multiple handlers at once, use an array');
		if (Array.isArray(handler)) {
			for (const h of handler) this.use(h);
			return this;
		}
		if (typeof handler !== 'function') throw new TypeError('Expected a handler function or array of such');
		this[shared.settings].handlers.push(handler);
		return this;
	}
}

const makeRoute = (handler, settings) => (req, res) => {
	const location = locate(req);
	if (!location || !handlers.length) return void req.destroy();
	const params = shared.params;
	shared.params = null;
	const request = new Request(req, location, params);
	// Run through handlers
	// After each handler, check if the request is aborted, and if the response is an error
	// Otherwise, keep on going
	// For each handler that returns a function, add it to an "after" queue
	// When an abortion or response occurs, callback
	const afters = [];
	invokeHandler(handler(afters), request, (response) => {
		
	});
};

const composeHandlers = (handlers) => (afters) => (request) => {
	let index = 1;
	return invokeHandler(handlers[0], function next(returned) {
		if (typeof returned === 'function') afters.push(returned);
		else if (returned != null) return castToResponse(returned);
		if (request.aborted) return { error: new Error('The request was aborted') };
		if (index === handlers.length) return { error: new TypeError(`Route handler did not return a response (got ${returned})`) };
		return invokeHandler(handlers[index++], request, next);
	});
};

const invokeAfters = (afters, response) => {
	let index = afters.length - 1;
	return invokeHandler(afters[index], function next(returned) {
		if (returned != null) {
			dropResponse(response);
			response = castToResponse(returned);
			if (response.error) return response;
		}
		if (request.aborted) {
			dropResponse(response);
			return { error: new Error('The request was aborted') };
		}
		if (index === 0) return response;
		return invokeHandler(afters[--index], response, next);
	});
};

const makeHandler = (beforeHandler, afterHandler, catcher, reporter) => (req, res) => {
	const location = locate(req);
	if (!location) return void req.destroy();
	const params = shared.params;
	shared.params = null;
	const request = new Request(req, location, params);
	/*
		The execution of a route is controlled by this state machine.
		0 = returned from the beforeHandler
		1 = returned from the catcher
		2 = returned from the afterHandler
		3 = an error occurred while responding via the catcher
		4 = an error occurred while responding via the afterHandler
		5 = the connection was aborted
	 */
	let state = 0;
	invokeMiddleware(beforeHandler, request, undefined, function route(response) {
		if (state < 2) {
			response = response == null
				? { error: new TypeError(`Expected route handler to return a response (got ${response})`) }
				: castToResponse(response);
		}
		if (request.aborted) {
			state = 5;
			if (!response.error) {
				dropResponse(response);
				response = { error: new Error('The request was aborted') };
			}
		}
		if (response.error) {
			if (!(state & 1)) {
				state = 1;
				invokeMiddleware(catcher, request, response.error, route);
			} else {
				if (reporter) process.nextTick(reporter, response.error);
				req.destroy();
			}
		} else if (state === 0) {
			state = 2;
			invokeMiddleware(afterHandler, request, response, route);
		} else {
			try { respond(req, res, response, reporter); }
			catch (error) { state += 2; route({ error }); }
		}
	});
};

const invokeHandler = (fn, arg, cb) => {
	let returned, sync;
	try {
		returned = fn(arg);
		sync = !Promise.isPromise(returned);
	} catch (err) {
		return cb(err);
	}
	if (sync) return cb(returned); // Avoid an unnecessary event loop cycle
	return Promise.resolve(returned).then(cb, cb);
};

const castToResponse = (value) => {
	if (typeof value === 'number' || Array.isArray(value)) {
		try { return new Response(value); }
		catch (err) { value = err; }
	}
	if (value instanceof Error) return { error: value };
	if (Response.isResponse(value)) return value;
	return { error: new TypeError(`Expected route response to be a number or array ${shared.hint(value)}`) };
};

const dropResponse = (response) => {
	if (River.isRiver(response.body)) response.body.pump(noop)();
};

const noop = () => {};
const passthrough = (a, b) => b;
const internalServerError = () => 500;
Object.setPrototypeOf(Route.prototype, Function.prototype);
module.exports = Route;
