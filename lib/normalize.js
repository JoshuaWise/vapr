...
module.exports = (req, response) => {
	...
	if (code === 204 || code === 304 || (head && legacy)) {
		...
		return { code, message, headers, body: undefined, trailers: undefined, streaming: false };
	}
	...
	if (!River.isRiver(body)) {
		if ((!trailers.size || legacy) && !transferEncoding) {
			...
			return { code, message, headers, body, trailers: undefined, streaming: false };
		}
		body = River.one(body);
	}
	...
	if (legacy) {
		...
		return { code, message, headers, body, trailers: undefined, streaming: true };
	}
	...
	return { code, message, headers, body, trailers, streaming: true };
};
