...
class Router {
	constructor(missingHandler, getStaticKey, getDynamicKeys) {
		...
		return Object.setPrototypeOf(makeHandler(this[statics], this[dynamics], this[missing], getStaticKey, getDynamicKeys), this);
	}
...
const makeHandler = (statics, dynamics, missing, getStaticKey, getDynamicKeys) => (req, res) => {
	const key = getStaticKey(req);
	if (!key) return void req.destroy();
	const handler = statics[key];
