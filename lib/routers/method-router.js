...
const addMethod = (self, method, handler) => {
	...
	if (headers.has(self)) {
		const header = headers.get(self);
		const methods = (header.allow ? header.allow.split(',') : []).concat(method).sort();
		header.allow = Buffer.from(methods.join(',')).toString(); // Serialize string in V8
	}
	return self;
};
