...
class PathRouter extends Router {
	...
	define(pathname, handler) {
		...
		if (typeof handler !== 'function') throw new TypeError('Expected path handler to be a function');
		...
		if (params.every(p => !p)) {
			...
			if (handler[shared.settings]) handler = wrapWithParams(handler, params.map((p, i) => ({ name: p, index: i + 1 })).filter(p => p.name));
...
// TODO: maybe save the processed pathname to the location object
