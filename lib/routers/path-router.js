...
class PathRouter extends Router {
	...
	define(pathname, handler) {
		...
		if (typeof handler !== 'function') throw new TypeError('Expected define() handler to be a function');
		...
		if (params.every(p => !p)) {
			...
			if (handler[shared.settings]) handler = wrapWithParams(handler, params.map((p, i) => ({ name: p, index: i })).filter(p => p.name));
...
const wrapWithParams = (handler, paramIndexes) => (req, res) => {
	// TODO: benchmark this
	const keys = locate(req).pathname.slice(1).split('/');
	const params = {};
	// TODO: should I decode keys[index] with decodeURIComponent? with decodeURI?
