...
class Response {
	...
	set code(value) { this[mut].code = normalizeCode(value); }
	set message(value) { this[mut].message = normalizeMessage(value); }
	set body(value) { this[mut].body = normalizeBody(value); }
	[inspect.custom]() {
...
const parseArray = (self, length, [code, message, headers, body]) => {
	...
	if (length > index) {
		throw new TypeError(`Unexpected value in response array[${index}]`);
	}
...
const normalizeMessage = (message, code) => {
	if (message != null && typeof message !== 'string') throw new TypeError('Expected response message to be a string');
