...
/*
	...
	can either be constructed from a status code number, an array, or an Error.
	...
	If the constructor argument is an Error object, the response will have a
	status code of 500, and the error will be available at response.error.
 */
...
class Response {
	constructor(value) {
		if (typeof value === 'number') {
			...
			this[mut] = { code: value, message, body: undefined, error: null };
		} else if (Array.isArray(value)) {
			parseArray(this, value.length, value);
		} else if (value instanceof Error) {
			this.headers = new Headers;
			this.trailers = new Trailers;
			this[mut] = { code: 500, message: message500, body: undefined, error: value };
		} else {
			throw new TypeError(`Expected response to be a number or array ${hint(value)}`);
		}
		Object.freeze(this);
	}
	get code() {
	...
	[inspect.custom]() {
		...
		for (const key of ['code', 'message', 'headers', 'trailers', 'body', 'error']) ret[key] = this[key];
		return ret;
	}
...
const parseArray = (self, length, [code, message, headers, body]) => {
	...
	self[mut] = { code, message, body: normalizeBody(body), error: null };
};
...
const normalizeBody = (body) => {
	...
	if (Promise.isPromise(body)) return Promise.resolve(body).catchLater();
	return body;
};
...
const message500 = codes.get(500);
