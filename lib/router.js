...
/*
	...
	key returned by getStaticKey(req). If an empty string is returned, no route
	will be matched. Static routes are prioritized over parameterized routes.
...
class Router {
	constructor(missingHandler, getStaticKey, getDynamicKeys) {
		...
		this[shared.instance] = this; // Provides write access for subclasses
		return Object.setPrototypeOf(makeHandler(this, getStaticKey, getDynamicKeys), this);
	}
	[shared.addStaticRoute](key, handler) {
		if (this[statics][key]) return false; // Prevent duplicate routes
	...
	[shared.setMissingRoute](handler) {
		if (this[changedMissing]) return false; // Only allow one invocation
