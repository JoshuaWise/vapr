...
/*
	...
	path is defined by a Resource (which is just a wrapper around MethodRouter).
	If a resource only has one method, shorthand functions can be used directly on
	the Application object (e.g., get(), post(), etc.).
...
class Application {
	...
	host(hostport, options) {
		const router = useHostRouter(this[settings]);
		const child = new Application(options == null ? this[settings].options : options);
		router.host(hostport, child);
		return child;
	}
	route(pathname, options) {
		const router = usePathRouter(this[settings]);
		const resource = new Resource(options == null ? this[settings].options : options);
