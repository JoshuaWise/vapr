...
module.exports = (req, res, response, reporter) => {
	...
	// attempts at responding to the same request will not be unable to include
	// a response body. We can fix the behavior by overriding the _hasBody field
	// on the http.ServerResponse object to have the correct value. However, if
	// we encounter a version of Node.js where that field does not exist, we
	// cannot predict the behavior and so it should be considered unsupported.
...
const tokenHeader = /^[-!#$%&'*+.^_`|~a-z0-9]+(?:[ \t]*,[ \t]*[-!#$%&'*+.^_`|~a-z0-9]+)*$/i;
const transferEncodingHeader = /^[-!#$%&'*+.^_`|~a-z0-9]+(?:[ \t]*;[ \t]*[-!#$%&'*+.^_`|~a-z0-9]+=(?:[-!#$%&'*+.^_`|~a-z0-9]+|"(?:[ \t\x21\x23-\x5b\x5d-\x7e\x80-\xff]|\\[ \t\x21-\x7e\x80-\xff])*"))*(?:[ \t]*,[ \t]*[-!#$%&'*+.^_`|~a-z0-9]+(?:[ \t]*;[ \t]*[-!#$%&'*+.^_`|~a-z0-9]+=(?:[-!#$%&'*+.^_`|~a-z0-9]+|"(?:[ \t\x21\x23-\x5b\x5d-\x7e\x80-\xff]|\\[ \t\x21-\x7e\x80-\xff])*"))*)*$/i;
