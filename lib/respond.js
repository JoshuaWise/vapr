...
/*
	...
	TODO: the Date and Connection headers are capitalized when added by `http`.
 */
...
module.exports = (req, res, response, logger) => {
	...
	if (body != null && !Buffer.isBuffer(body) && !River.isRiver(body) && typeof body !== 'string') {
		throw new TypeError(`Expected response body to be a buffer, string, river, or null ${hint(body)}`);
	}
...
const stream = (res, body, trailers, logger) => {
	...
	const cancel = body.pump((chunk) => {
		if (Buffer.isBuffer(chunk) || typeof chunk === 'string') res.write(chunk);
		else if (chunk != null) throw new TypeError(`Expected response body to be made of buffers or strings ${hint(chunk)}`);
	});
	res.on('close', () => {
		if (dead) return;
		dead = true;
