...
class HostRouter extends Router {
	constructor({ defaultPort: port = 80 } = {}) {
		if (typeof port !== 'number') throw new TypeError('Expected defaultPort to be a number');
		if (port >>> 0 !== port || port > 65535) throw new TypeError(`Invalid default port: ${port}`);
		super(badGateway, getStaticKey, getDynamicKeys);
		this[shared.instance][defaultPort] = ':' + port;
	}
	host(hostport, handler) {
		...
		if (!hostport.includes(':')) hostport += this[defaultPort];
...
const getStaticKey = (req) => {
	...
	if (!location) return '';
	if (wontMatch(location.host)) return '.0'; // Bypass unsupported hosts
	return `${location.host}:${location.port}`;
};

const getDynamicKeys = (key) => {
	const [host, port] = key.split(':');
	if (!notIP(host)) return; // IP addresses cannot be parameterized
	const keys = host.split('.');
