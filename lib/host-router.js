...
/*
	...
	TODO: domain names and subdomains can actually start with a digit
	TODO: hostnames can be IP addresses
 */
...
const splitKey = (key) => {
	if (key.length > 254 || !validHost.test(key)) return;
...
const validDomain = /^(?=[^:]{1,253}(?::|$))(?:(?:\*|[a-z](?:[a-z0-9-]{0,61}[a-z0-9])?)(?:\.(?!$)|(?::\d+)?$)){2,}$/i;
const validHost = /^(?:[a-z](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.|(?=:))){2,}:\d+$/i;
