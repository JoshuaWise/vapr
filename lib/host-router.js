...
const findHostFn = (router, location, req) => {
  ...
  // Then, check if there's an exact match or pattern match.
  return matchPort(router[hosts][hostname], port, localPort)
    || matchSubhost(router[patterns], hostname, port, localPort)
    || router[noHost];
}
