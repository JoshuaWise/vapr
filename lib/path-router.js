...
class PathRouter extends Router {
	...
	define(method, pathname, handler) {
		if (method === 'CONNECT') { handler = pathname; pathname = ''; var skip = true; }
		...
		if (!skip && !validRoute.test(pathname)) throw new TypeError(`Invalid pathname: ${pathname}`);
		pathname = this[normalizer](pathname);
		if (!hasParam.test(pathname)) {
...
// TODO: decode path segments before matching, with decodeURIComponent() (this can throw)
const getStaticKey = (normalize) => (req) => {
	const location = locate(req);
	if (!location) return '';
	if (!location.scheme) return req.method === 'CONNECT' && location.host ? 'CONNECT' : '';
	return req.method + normalize(location.path);
};

const getDynamicKeys = (key) => {
	const index = key.indexOf('/');
	if (index === key.length - 1 || index === -1) return;
	return key.split('/');
};
