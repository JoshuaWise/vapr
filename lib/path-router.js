...
const splitKey = (key) => {
	if (key.length > 2111 || !validDynamicKey.test(key)) return;
	const keys = key.split('/');
	if (key.length - keys[0].length > 2048) return;
	return keys;
};
...
const validRoute = /^(?=.{1,2048}$)(?:(?:\/(?::[a-z_$][a-z_$0-9]*|(?:[a-z0-9_$()+,-]|%[0-9a-f]{2})+))+\/?|[*/])$/i;
const validDynamicKey = /^[^/*]+(?:\/(?:[a-z0-9_$()+,-]|%[0-9a-f]{2})+)+\/?$/i;
