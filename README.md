...
In Vapr, a response is generated by returning or throwing a value from the route handler or a middleware plugin. If the value is a number, it will generate an empty response with that status code.
...
```js
child1.get('/foo', () => { /* respond here */ });
child2.get('/foo', () => { /* respond here */ });
child3.get('/foo', () => { /* respond here */ });
```
