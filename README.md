...
Vapr's router is different from most. Most routers work by linearly scanning a list of regular expressions until a matched is found. This is very inefficient, and scales poorly as your application gets bigger. Vapr's router works by traversing a [radix tree](https://en.wikipedia.org/wiki/Radix_tree), which is performed in *constant time*. This doesn't matter for small applications, but it could be important for very large applications. Vapr doesn't want to weigh you down, regardless of how big your application is.
